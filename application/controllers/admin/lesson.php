...
class Lesson extends MY_Controller {
	...
	public function index() {
		...
		$data = array();
		$data['category'] = $this->lessondb->get_category();
		$this->Render('lesson','lesson','Lesson', $data);
	}
	...
	function process() {
		...
		if($post['id'] == 'new') {
			...
			if($this->lessondb->get_lesson_by_name($post['lesson_name']) != null) {
				echo json_encode(
					array(
						'error_code'=>1,
			...
			$this->lessondb->insert_lesson($post);
			...
			$this->lessondb->update_lesson($post);
	...
	function get_by_id() {
		$post = $this->PopulatePost();
		$this->load->model('lessondb');
		$obj = $this->lessondb->get_lesson_by_id($post['lesson_id']);
		if($obj != null) {
	...
	function delete() {
		...
		$post = $this->PopulatePost(); 
		$obj = $this->lessondb->get_lesson_by_id($post['data']);

		if($obj != null) { 

			$this->lessondb->delete_lesson($post['data']);
			echo json_encode(
				array(
					'error_code'=>'0',
